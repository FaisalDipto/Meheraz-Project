...
export default function GetStarted() {
  ...
  // Steps: 'agreements' | 'pricing' | 'connect'
  const [currentStep, setCurrentStep] = useState(isReturningUser ? 'connect' : 'agreements');
  const [agreed, setAgreed] = useState(false);

  useEffect(() => {
    // If returning user, skip to connect step
    if (isReturningUser) {
        ...
        <div className="onboarding-modal-overlay">
          <div className="onboarding-modal-container fade-in">
            <div className="onboarding-modal-body custom-scrollbar">
              <div className="mb-8">
                <h2 className="modal-step-title">Agreement to Terms</h2>
                <p className="modal-step-desc">Please read and agree to our Terms of Service and Privacy Policy before continuing.</p>
              </div>
              <LegalCenter />
            </div>
            <div className="onboarding-modal-footer">
              ...
              <button 
                className={`btn-step-next ${!agreed ? 'opacity-50 cursor-not-allowed' : ''}`}
